...
{
    static uint8_t rete = 0;
    
    rete  = mpu_init(NULL);                                      /* 硬件初始化 */
    rete += mpu_set_sensors(INV_XYZ_GYRO | INV_XYZ_ACCEL);       /* 开启指定传感器 */
    rete += mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL);    /* 设置FIFO */
    rete += mpu_set_sample_rate(DEFAULT_MPU_HZ);                 /* 设置采样率 */
    rete += dmp_load_motion_driver_firmware();                   /* 加载DMP镜像 */
    rete += dmp_set_orientation(                                 /* 设置陀螺仪方向 */
                                inv_orientation_matrix_to_scalar(gyro_orientation));
    rete += dmp_enable_feature(  DMP_FEATURE_6X_LP_QUAT      |   /* 设置DMP功能 */
                                DMP_FEATURE_TAP             |
                                DMP_FEATURE_ANDROID_ORIENT  |
                                DMP_FEATURE_SEND_RAW_ACCEL  |
                                DMP_FEATURE_SEND_CAL_GYRO   |
                                DMP_FEATURE_GYRO_CAL);
    rete += dmp_set_fifo_rate(DEFAULT_MPU_HZ);                   /* 设置DMP输出速率 */
    rete += mpu_set_dmp_state(1);                                /* 使能DMP */
    rete += atk_ms6050_run_self_test();                          /* 传感器自测试 */
